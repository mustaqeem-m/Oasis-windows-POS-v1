...
class ServiceStaff extends Api {
  var serviceStaff;

  Future<List> get() async {
    try {
      String url = this.apiUrl + "user?service_staff=1";
      var token = await System().getToken();
      var response =
          await http.get(Uri.parse(url), headers: this.getHeader('$token'));
      serviceStaff = jsonDecode(response.body);
      var serviceStaffList = serviceStaff['data'];
      System().insert('serviceStaff', jsonEncode(serviceStaffList));
