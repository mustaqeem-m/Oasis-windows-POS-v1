...
<html lang="en">
    ...
    <body>
        <div class="ticket">
			...
			@endif
      <table style="margin-top: 25px !important; width: 100%;" class="border-bottom width-100 table-f-12 mb-10">
    <thead class="border-bottom-dotted">
        <tr>

            <th style="width: 5%; text-align: center;">#</th>
<th class="description" style="width: 34%;">{{ $receipt_details->table_product_label }}</th>
<th style="width: 12%; text-align: right;">Qty</th>
<th style="width: 14%; text-align: right;">&nbsp;MRP</th>
...
<tbody>
    @php
        // Initialize running totals (if you need them later)
        $total_quantity = 0;
        $total_mrp      = 0;
        $total_our_price= 0;
        $total_discount = 0;
    ...
    @foreach($receipt_details->lines as $line)
        @php
            // helper to strip commas & cast
            $parse = fn($v) => (float) str_replace(',', '', $v);

            // 1. Quantity
            $qty = $parse($line['quantity'] ?? 0);

            // 2. MRP only from custom-fields
            $mrp = 0;
            if (!empty($line['product_custom_fields'])) {
                // if it's an array, take the first value
                $raw = is_array($line['product_custom_fields'])
                         ? reset($line['product_custom_fields'])
                         : $line['product_custom_fields'];
                $mrp = $parse($raw);
            }

            // 3. Our Price (prefer the _uf override if present)
            if (!empty($line['unit_price_inc_tax_uf'])) {
                $ourPrice = $parse($line['unit_price_inc_tax_uf']);
            } elseif (!empty($line['unit_price_inc_tax'])) {
                $ourPrice = $parse($line['unit_price_inc_tax']);
            } else {
                $ourPrice = $parse($line['unit_price'] ?? 0);
            }

            // 4. Price excl. tax (for taxable total)
            $excTax = $parse($line['price_exc_tax'] ?? $ourPrice);

            // 5. Discount per unit (never negative)
            $unitDiscount = max($mrp - $ourPrice, 0);

            // 6. Line totals
            $lineTotal    = $ourPrice * $qty;
            $lineDiscount = $unitDiscount * $qty;

            // 7. Accumulate running totals
            $total_quantity   += $qty;
            $total_mrp        += $mrp * $qty;
            $total_our_price  += $ourPrice * $qty;
        ...
        <tr>
            ...
            <td style="text-align: right; vertical-align: top;">
                {{ number_format($qty, 2) }} {{ $line['units'] }}
            </td>
            <td style="text-align: right; vertical-align: top;">
                {{ $mrp > 0 ? number_format($mrp, 2) : '-' }}
            </td>
            <td style="text-align: right; vertical-align: top;">
    ...
    $subtotal_mrp = 0;
    foreach($receipt_details->lines as $line) {
        // Get MRP from product custom fields (first field)
        $mrp = 0;
        if (!empty($line['product_custom_fields'])) {
            $raw = is_array($line['product_custom_fields']) ? reset($line['product_custom_fields']) : $line['product_custom_fields'];
            $mrp = (float) str_replace(',', '', $raw);
        }
        $qty = (float) str_replace(',', '', $line['quantity'] ?? 0);
        $subtotal_mrp += $mrp * $qty;
    }
    $grand_total = (float) str_replace(',', '', $receipt_details->total);
    $total_saved = $subtotal_mrp - $grand_total;
@endphp

<!-- Display Total Saved only if positive -->
<!--@if($total_saved > 0)-->
<!--<div class="flex-box">-->
<!--    <p class="left text-right">-->
...
<!--@endif-->
				...
				<div class="flex-box">
					<p class="width-50 text-right sub-headings">
						...
						<!--<br>DEBUG VALUE: {{$receipt_details->total}}-->
					</p>
					<p class="width-50 text-right sub-headings">
