...
<html lang="en">
    ...
    <body>
        <div class="ticket">
			...
			@endif
            <table style="margin-top: 10px !important; border-collapse: collapse; width: 100%; table-layout: fixed;" class="border-bottom table-f-12 mb-10">
                <thead class="border-bottom-dotted">
                    <tr>
                        <th class="serial_number" style="text-align:center; width:6%">#</th>
                        <th class="description" style="text-align:left; width:50%">{{$receipt_details->table_product_label}}</th>
                        <th class="quantity" style="text-align:center; width:10%">{{$receipt_details->table_qty_label}}</th>
                        @if(empty($receipt_details->hide_price))
                        <th class="unit_price" style="text-align:right; width:16%">{{$receipt_details->table_unit_price_label}}</th>
                        @if(!empty($receipt_details->item_discount_label))
                            <th class="text-right" style="width:12%">{{$receipt_details->item_discount_label}}</th>
                        @endif
                        <th class="price text-right" style="width:24%">{{$receipt_details->table_subtotal_label}}</th>
                        @endif
                    </tr>
                </thead>
                <tbody>
                	@forelse($receipt_details->lines as $line)
	                    <tr>
	                        <td class="serial_number" style="vertical-align: top; text-align:center;">{{$loop->iteration}}</td>
	                        <td class="description" style="text-align:left;">
	                            ...
	                            @endif
	                        	@if(!empty($line['sell_line_note']))<br><span class="f-8">{!!$line['sell_line_note']!!}</span>@endif 
	                        ...
	                        <td class="quantity" style="text-align:center; vertical-align: top;">
	                            {{$line['quantity']}}
	                            @if($receipt_details->show_base_unit_details && $line['quantity'] && $line['base_unit_multiplier'] !== 1)
	                        ...
	                        @if(empty($receipt_details->hide_price))
	                        <td class="unit_price" style="text-align:right; vertical-align: top;">{{$line['unit_price_inc_tax']}}</td>
	                        @if(!empty($receipt_details->item_discount_label))
	                        ...
	                        @endif
	                        <td class="price text-right" style="vertical-align: top;">{{$line['line_total']}}</td>
	                        @endif
	                    </tr>
							...
							@foreach($line['modifiers'] as $modifier)
								<tr>
									...
									<td>
			                            ...
			                            @if(!empty($modifier['sub_sku'])), {{$modifier['sub_sku']}} @endif @if(!empty($modifier['cat_code'])), {{$modifier['cat_code']}}@endif @if(!empty($modifier['sell_line_note']))({!!$modifier['sell_line_note']!!}) @endif 
			                        </td>
									<td class="text-center">{{$modifier['quantity']}} {{$modifier['units']}}</td>
									@if(empty($receipt_details->hide_price))
									<td class="text-right">{{$modifier['unit_price_inc_tax']}}</td>
									@if(!empty($receipt_details->item_discount_label))<td class="text-right">0.00</td>@endif
...
.serial_number, .quantity, .unit_price, .price {
